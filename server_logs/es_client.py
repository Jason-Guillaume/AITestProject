...
def get_elasticsearch_client():
    ...
    url = str(getattr(settings, "ELASTICSEARCH_URL", "http://localhost:9200") or "").strip()
    if not url:
    ...
    timeout = int(getattr(settings, "ELASTICSEARCH_REQUEST_TIMEOUT", 5) or 5)

    kwargs: dict[str, Any] = {"verify_certs": verify, "request_timeout": timeout}
    if password:
