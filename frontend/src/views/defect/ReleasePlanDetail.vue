  ...
  <div class="release-detail cyber-page">
    <div class="detail-header">
      <div class="detail-header__left">
        ...
        <el-tag
          :type="releaseTag(detail.status)"
          size="small"
        >
          {{ releaseLabel(detail.status) }}
        </el-tag>
      </div>
        ...
        <template v-else-if="briefData">
          ...
          >
            <el-descriptions-item label="关联用例">
              {{ briefData.coverage?.linked_cases ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="测试计划（总数）">
              {{ briefData.coverage?.test_plans?.total ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="计划·未开始">
              {{ briefData.coverage?.test_plans?.not_started ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="计划·进行中">
              {{ briefData.coverage?.test_plans?.in_progress ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="计划·已完成">
              {{ briefData.coverage?.test_plans?.completed ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="缺陷·未关闭">
              {{ briefData.defects?.open_not_closed ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="缺陷·总数">
              {{ briefData.defects?.total ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="API 执行·窗口内总次数">
              {{ briefData.executions?.total_runs ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="API 执行·通过">
              {{ briefData.executions?.passed ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="API 执行·未通过">
              {{ briefData.executions?.failed ?? 0 }}
            </el-descriptions-item>
            <el-descriptions-item label="近窗从未执行用例数">
              {{ briefData.executions?.never_executed_cases ?? 0 }}
            </el-descriptions-item>
          </el-descriptions>
          <div
            class="panel-title"
          ...
          <div class="risk-toolbar">
            <el-button
              ...
              @click="loadNeverExecuted"
            >
              加载清单
            </el-button>
            <el-button
              size="small"
              :disabled="neverExporting"
              :loading="neverExporting"
              @click="exportNeverCsv"
            >
              导出 CSV
            </el-button>
            <span
              v-if="neverTotal != null"
              class="never-total"
            >合计 {{ neverTotal }} 条（列表最多展示 500 条）</span>
          </div>
          <el-table
