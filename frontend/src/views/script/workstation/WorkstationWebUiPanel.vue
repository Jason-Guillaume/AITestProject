<template>
  <div class="layout sw-cyber">
    <aside class="sw-cyber-panel col-tree">
      <div class="panel-head sw-glow-text">POM / 工程结构</div>
      <el-tree
        :key="wb.uiTreeKey"
        class="tree"
        lazy
    ...
    <div class="col-main">
      <div class="sw-cyber-panel is-active row-toolbar">
        <el-button class="ghost-btn" :icon="Plus" @click="wb.openImport">导入</el-button>
        <el-button class="ghost-btn" :loading="wb.saveLoading" :disabled="!wb.canSaveEditor" @click="wb.saveEditor">保存</el-button>
        <el-button class="ghost-btn accent" :icon="VideoPlay" :disabled="!hasProject" :loading="ui.isRunning" @click="wb.runExecution">
          {{ ui.isRunning ? '执行中' : '执行' }}
        </el-button>
        <el-button
      ...
      <div class="sw-cyber-panel row-editor">
        <div class="exec-row">
          ...
          <el-select v-model="wb.executionConfig.browser" size="small" class="mini-sel">
            <el-option label="Chrome" value="chrome" />
            <el-option label="Firefox" value="firefox" />
            <el-option label="Edge" value="edge" />
          </el-select>
          <el-radio-group v-model="wb.executionConfig.headless" size="small">
            <el-radio-button :label="false">有头</el-radio-button>
            <el-radio-button :label="true">无头</el-radio-button>
          </el-radio-group>
          <span class="muted">并发</span>
          <el-input-number v-model="wb.executionConfig.parallel" :min="1" :max="10" size="small" />
        </div>
        <p class="path sw-glow-text">{{ wb.editorTitle || '未选择文件' }}</p>
        <div v-loading="wb.editorLoading" class="monaco-box">
      ...
      <div class="sw-cyber-panel row-pipeline">
        <div class="panel-head sw-glow-text">截图 / 录屏流水线</div>
        <el-steps :active="pipelineActive" simple class="steps">
          <el-step title="用例开始" />
        ...
        <div class="preview-strip">
          <div v-for="i in 4" :key="i" class="thumb sw-cyber-panel">
            <span class="thumb-label">Step {{ i }}</span>
            <el-empty description="等待执行产出" :image-size="48" />
          </div>
        </div>
      </div>

      <el-tabs v-model="wb.mainTab" class="bottom-tabs">
        <el-tab-pane label="最近报告" name="report">
          <div class="report-grid">
            <div class="sw-cyber-panel report-side">
              <div class="panel-head">POM 归档</div>
              <el-table
              ...
              >
                ...
                <el-table-column prop="execution_status" label="状态" width="64" />
              </el-table>
            </div>
            <div class="sw-cyber-panel report-frame">
              <iframe v-if="wb.reportIframeUrl" class="iframe" title="r" :src="wb.reportIframeUrl" />
              <el-empty v-else description="选中表格行预览 HTML" :image-size="64" />
            </div>
          </div>
        </el-tab-pane>
        <el-tab-pane label="终端" name="terminal">
          <div :ref="(el) => bindTermScroll(el as HTMLElement | null)" class="term sw-cyber-panel">
            <template v-if="ui.logs.length === 0">
              <span class="term-idle">$ idle</span>
            </template>
            <div v-for="(log, idx) in ui.logs" :key="idx" class="term-line">{{ log.time }} {{ log.message }}</div>
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
