/**
 * Web UI 工作台（/automation-center/webui）模块级 E2E：登录后校验布局、指挥中心入口、工作台关键控件。
 *
...
test.describe("WebUI 工作台模块", () => {
  ...
  test("自动化指挥中心页可访问", async ({ page }) => {
    await page.goto("/automation-center", { waitUntil: "domcontentloaded" });
    await expect(page).not.toHaveURL(/\/login(\b|\/|\?|#)/);
    await expect(page.locator(".script-hub--cyber")).toBeVisible({ timeout: 15_000 });
    await expect(page.getByText("Platform Adapter")).toBeVisible();
  });

  test("Web UI 工作台：面包屑、侧栏、导入与执行控制台", async ({ page }) => {
    await page.goto("/automation-center/webui", { waitUntil: "domcontentloaded" });
    await expect(page).not.toHaveURL(/\/login(\b|\/|\?|#)/);
